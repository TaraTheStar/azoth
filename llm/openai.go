...
// Debug logging.
//
// We write to a file rather than stderr because stderr in a TUI-driven
// terminal session interleaves with the UI's own draw calls and visibly
// corrupts the screen. Tail the file in another pane during diagnosis.
//
// Opt-in per app: call SetDebug with a path (e.g. from a --debug flag or
// a debug env var the app defines).
var (
...
type OpenAIClient struct {
	...
	// ConnectHint, when non-empty, is appended to transport-failure
	// messages (ConnectError) so each app can point the user at its own
	// config ("check base_url in ~/.config/<app>/config.toml"). Purely
